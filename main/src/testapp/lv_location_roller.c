 ...
 *      DEFINES
 *********************/
#define LV_LOCATION_ROLLER_FLAG_AUTO_PARENT  LV_OBJ_FLAG_WIDGET_1        /*Mark that the parent was automatically created*/
#define MY_CLASS    &lv_location_roller_class
 ...
 *  STATIC PROTOTYPES
 **********************/
static void location_roller_close_click_event_cb(lv_event_t * e);

/**********************
 *  STATIC VARIABLES
 **********************/
const lv_obj_class_t lv_location_roller_class = {
    .base_class = &lv_obj_class,
    .width_def = LV_DPI_DEF * 2,
    .height_def = LV_SIZE_CONTENT,
    .instance_size = sizeof(lv_location_roller_t)
};

const lv_obj_class_t lv_location_roller_content_class = {
    .base_class = &lv_obj_class,
...
const lv_obj_class_t lv_location_roller_backdrop_class = {
    .base_class = &lv_obj_class,
    .width_def = LV_PCT(100),
                            ...
                            bool add_close_btn)
{
    ...
    if(parent == NULL) {
        auto_parent = true;
        parent = lv_obj_class_create_obj(&lv_location_roller_backdrop_class, lv_layer_top());
    ...
    lv_obj_t * obj = lv_obj_class_create_obj(&lv_location_roller_class, parent);
    LV_ASSERT_MALLOC(obj);
    lv_obj_class_init_obj(obj);
    if(obj == NULL) return NULL;
    lv_location_roller_t * mbox = (lv_location_roller_t *)obj;

    if(auto_parent) lv_obj_add_flag(obj, LV_LOCATION_ROLLER_FLAG_AUTO_PARENT);
    ...
    if(add_close_btn) {
        ...
        lv_obj_add_event_cb(mbox->close_btn, location_roller_close_click_event_cb, LV_EVENT_CLICKED, NULL);
    ...
    mbox->content = lv_obj_class_create_obj(&lv_location_roller_content_class, obj);
...
{
    lv_obj_t * btnm = lv_location_roller_get_btns(mbox);
...
{
    if(lv_obj_has_flag(mbox, LV_LOCATION_ROLLER_FLAG_AUTO_PARENT)) lv_obj_del(lv_obj_get_parent(mbox));
    else lv_obj_del(mbox);
}

void lv_location_roller_close_async(lv_obj_t * dialog)
{
    if(lv_obj_has_flag(dialog, LV_LOCATION_ROLLER_FLAG_AUTO_PARENT)) lv_obj_del_async(lv_obj_get_parent(dialog));
