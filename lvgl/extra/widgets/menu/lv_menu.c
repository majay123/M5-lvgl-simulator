...
//2023-05-16 add by RunningCode  
lv_obj_t * lv_menu_item_create(lv_obj_t * parent)
{
    LV_LOG_INFO("begin");
   ...
   // lv_obj_set_height(obj, 70);
    lv_obj_set_style_bg_color(obj, lv_color_hex(0x29292b), LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_bg_color(obj, lv_color_hex(0x434449), LV_PART_MAIN | LV_STATE_CHECKED);
...
void lv_menu_set_sidebar_page(lv_obj_t * obj, lv_obj_t * page)
{
    ...
    if(page != NULL) {
        ...
        if(!menu->sidebar_generated) {
           ...
           //2023-05-17 add by RunningCode  
            lv_obj_set_pos(sidebar_cont, 0, 0);
            lv_obj_set_size(sidebar_cont, 150, 480);
            lv_obj_set_style_pad_all(sidebar_cont, 0, 0);
            lv_obj_set_style_border_width(sidebar_cont, 0, 0);
            lv_obj_set_scrollbar_mode(sidebar_cont, LV_SCROLLBAR_MODE_OFF);
            ...
            //当设置数量较少时，禁止滑动
            lv_obj_clear_flag(sidebar_cont, LV_OBJ_FLAG_SCROLLABLE);
          ...
          //  lv_obj_set_size(sidebar_header, LV_PCT(100), LV_SIZE_CONTENT);
            lv_obj_set_size(sidebar_header, 150, 58);
            ...
            //2023-05-17 add by RunningCode  
            // lv_obj_set_pos(sidebar_header_back_btn, 0, 0);
            // lv_obj_set_style_pad_all(sidebar_header_back_btn, 0, 0);
            // lv_obj_set_style_border_width(sidebar_header_back_btn, 0, 0);
...
void lv_menu_set_load_page_event(lv_obj_t * menu, lv_obj_t * obj, lv_obj_t * page)
{
    ...
    lv_obj_set_style_bg_color(obj, lv_color_hex(0xFFFFFF), 0 | LV_STATE_CHECKED);
...
static void lv_menu_constructor(const lv_obj_class_t * class_p, lv_obj_t * obj)
{
    ...
    menu->main_header_title = main_header_title;
      //2023-05-16 add by RunningCode  
    lv_obj_set_style_text_color(menu->main_header_title, lv_color_hex(0xffffff), 0);
    menu->main_page = NULL;
...
static void lv_menu_value_changed_event_cb(lv_event_t * e)
{
    ...
    if(sidebar_page != NULL && menu->sidebar_header_title != NULL) {
        if(sidebar_page->title != NULL) {
            ...
            //2023-05-16 add by RunningCode   返回按钮，修改颜色
            lv_obj_set_style_text_color(menu->sidebar_header_title,  lv_color_hex(0xffffff), 0);
