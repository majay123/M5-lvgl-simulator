   ...
   lv_coord_t h = lv_obj_get_content_height(cont);
    if(lv_obj_get_style_base_dir(obj, LV_PART_MAIN) != LV_BASE_DIR_RTL) {
       ...
       // lv_obj_scroll_to_x(cont, (gap + w) * id_rtl, anim_en);
        lv_obj_scroll_to_y(cont, (gap + h) * id_rtl, anim_en);
    }
...
static void cont_scroll_end_event_cb(lv_event_t * e)
{
    ...
    if(code == LV_EVENT_LAYOUT_CHANGED) {
        ...
        //lv_coord_t w = lv_obj_get_content_width(cont);
        lv_coord_t h = lv_obj_get_content_height(cont);
        // lv_coord_t s = lv_obj_get_scroll_y(cont);
        // lv_coord_t y = lv_obj_get_scroll_top(cont);
        // lv_coord_t b = lv_obj_get_scroll_bottom(cont);
        
        lv_coord_t t;

        //  if(lv_obj_get_style_base_dir(tv, LV_PART_MAIN) == LV_BASE_DIR_RTL)  t = -(p.x - w / 2) / w;
        //  else t = (p.x + w / 2) / w;
        if(lv_obj_get_style_base_dir(tv, LV_PART_MAIN) == LV_BASE_DIR_RTL)  t = -(p.y - h / 2) / h;
        else t = (p.y + h / 2) / h;
        ...
        // printf("goto settings:s   %d \n",s);
        // printf("goto settings:y  %d \n",y);
        // printf("goto settings:b  %d \n",b);
        bool new_tab = false;
