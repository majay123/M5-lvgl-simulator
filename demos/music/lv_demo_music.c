...
{
    //lv_scr_act默认图层 ，这里有top和sys图层，用于提示，或者对话框做图层显示
    lv_obj_set_style_bg_color(lv_scr_act(), lv_color_hex(0x343247), 0);//设置背景颜色

    list = _lv_demo_music_list_create(lv_scr_act()); //先创建，在下层
    ctrl = _lv_demo_music_main_create(lv_scr_act());//后创建，在上层
    //my_test = _lv_demo_music_main_create1(lv_scr_act());
